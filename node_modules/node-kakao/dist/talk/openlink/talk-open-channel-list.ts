...
export class TalkOpenChannelList
  ...
  async chatOffChannel() : AsyncCommandResult {
    const res = await this._manageSession.chatOffChannel();
  ...
  createOpenDirectChannel(
    template: OpenLinkChannelTemplate & OpenLinkCreateTemplate,
    profile: OpenLinkProfiles
  ): AsyncCommandResult<InformedOpenLink> {
    return this._linkStore.createOpenDirectChannel(template, profile);
  }
  ...
  static async initialize(
    ...
    await Promise.all(channelList.map((data) => {
      talkChannelList.addOpenChannel(data.channel, data.lastUpdate);
      talkChannelList.chatOffChannel();
    }));
