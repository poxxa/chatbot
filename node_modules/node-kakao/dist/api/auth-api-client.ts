...
import { AndroidSubXVCProvider, XVCProvider } from './xvc';
...
export class AuthApiClient {
  ...
  async login(form: LoginForm): AsyncCommandResult<LoginData> {
    const res = await this._client.requestData(
      ...
      this.fillAuthForm({ ...form, forced: false }),
  ...
  async registerDevice(form: LoginForm, passcode: string): AsyncCommandResult {
    const res = await this._client.requestData(
      ...
      this.fillAuthForm({ ...form, passcode, model_name: this.config.deviceModel, forced: false }),
      await this.createAuthHeader(form),
    );
  ...
  static async create(
    ...
    return new AuthApiClient(
      ...
      name,
      deviceUUID,
      advertisementId ? advertisementId : crypto.randomUUID(),
      Object.assign({ ...DefaultConfiguration }, config),
      xvcProvider ? xvcProvider : AndroidSubXVCProvider
    );
  }
}
