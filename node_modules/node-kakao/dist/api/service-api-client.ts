...
export class ServiceApiClient {
  ...
  async requestMoreSettings(since = 0): AsyncCommandResult<MoreSettingsStruct> {
    let vc = '';
    if(this.config.version === '9.8.9')
      vc = `&vc=2409890`;
    else if(this.config.version === '9.9.3')
      vc = `&vc=2409930`
    const res = await this._client.requestData(
      'GET',
      // eslint-disable-next-line max-len
      `${this.getAccountApiPath('more_settings.json')}?os_version=25&model=${this.config.deviceModel}&since=${encodeURIComponent(since)}&lang=${encodeURIComponent(this.config.language)}&email=2${vc}`,
    );
  ...
  private getQRCodeApiPath(api: string) {
    return `${this.config.agent}/qrcode/${api}`;
  }
