...
(function (factory) {
     ...
     */
    class AuthApiClient {
        constructor(client, _name, _deviceUUID, _advertisementId, config, xvcProvider) {
            this._name = _name;
            this._deviceUUID = _deviceUUID;
            this._advertisementId = _advertisementId;
            this.config = config;
        ...
        fillAuthForm(form) {
            form['one_store'] = false;
            form['permanent'] = true;
            form['device_uuid'] = this._deviceUUID;
            form['device_name'] = this._name;
            form['device_name'] = this.config.deviceModel;
            form['forced'] = false;
            return form;
        }
        /**
         ...
         */
        async login(form) {
            const res = await this._client.requestData('POST', this.getApiPath('login.json'), this.fillAuthForm({ ...form, forced: false }), await this.createAuthHeader(form));
        ...
        async requestPasscode(form) {
            const res = await this._client.requestData('POST', this.getApiPath('request_passcode.json'), this.fillAuthForm({ ...form, forced: false }), await this.createAuthHeader(form));
         ...
         */
        async registerDevice(form, passcode) {
            const res = await this._client.requestData('POST', this.getApiPath('register_device.json'), this.fillAuthForm({ ...form, passcode, model_name: this.config.deviceModel, forced: false }), await this.createAuthHeader(form));
            return { status: res.status, success: res.status === request_1.KnownDataStatusCode.SUCCESS };
        }
         ...
         */
        static async create(name, deviceUUID, advertisementId, config = {}, xvcProvider) {
            return new AuthApiClient(await (0, web_client_1.createWebClient)('https', 'katalk.kakao.com'), name, deviceUUID, advertisementId ? advertisementId : crypto_1.default.randomUUID(), Object.assign({ ...config_1.DefaultConfiguration }, config), xvcProvider ? xvcProvider : xvc_1.AndroidSubXVCProvider);
        }
    }
